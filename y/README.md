...
$ go install github.com/intex-software/goutils/y@latest
```
...
import (
	...
	"intex.software/intexforms/forms/dto"
	"intex.software/intexforms/lib/golang"
)

func New() (t *interp.Interpreter, err error) {
	t = interp.New(interp.Options{Unrestricted: false})
	t.Use(symbols.Symbols)
	t.ImportUsed()

	return
}

func CreateYaegiAdapter(source string) (adapter Adapter, err error) {
	t, err := golang.New()
	if err != nil {
		return
	}

	if _, err = t.Eval(source); err != nil {
		err = fmt.Errorf("error (%w) during evaluation", err)
		return
	}

	var v reflect.Value
	if v, err = t.Eval("main.Adapter"); err != nil {
		return
	} else if functor, ok := v.Interface().(func(string, string, dto.OciCallPosition) (map[string]any, error)); !ok {
		err = fmt.Errorf("oci adapter has the wrong interface: '%T' != 'Adapter'", v.Interface())
		return
	} else {
		adapter = toAdapter(functor)
	}
	return
}
```
...
```go
//go:generate yaegi extract -name symbols -include OciCallPosition,Out,Outf intex.software/intexforms/forms/dto intex.software/intexforms/log
...
```go
// Code generated by 'yaegi extract intex.software/intexforms/log'. DO NOT EDIT.

package symbols

import (
	"intex.software/intexforms/log"
	"reflect"
)

func init() {
	Symbols["intex.software/intexforms/log/log"] = map[string]reflect.Value{
		// function, constant and variable definitions
		"Out":  reflect.ValueOf(log.Out),
		"Outf": reflect.ValueOf(log.Outf),
	}
}
```
